...
/*
【今回このファイルで修正したこと】
1. shouldHoldBlankThreadStage=true のときに rendered を空配列へ差し替える処理を削除しました。
2. shouldHoldBlankThreadStage=true のときに visibleTexts を空 Map へ差し替える処理を削除しました。
3. rendered が存在する場合は、shouldHoldBlankThreadStage を false 扱いにして本文 stream を優先するようにしました。
4. shouldHoldBlankThreadStage=true のときに shouldShowPreparing=true へ強制する処理を削除しました。
5. 親から渡された shouldShowWorkspaceHero はそのまま ChatMessagePane へ渡すようにしました。
6. 本文採用、confirmed payload、state_changed、HOPY回答○、Compass、DB保存/復元、1..5 の唯一の正には触っていません。
*/
