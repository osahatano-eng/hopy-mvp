...
export default function LeftRailAccountSection({ userState }: Props) {
  ...
  React.useEffect(() => {
    ...
    const syncBottomUserInfoOnVisible = () => {
    ...
    if (typeof window !== "undefined") {
      window.addEventListener("focus", syncBottomUserInfo);
      window.addEventListener("pageshow", syncBottomUserInfo);
    }

    if (typeof document !== "undefined") {
      document.addEventListener("visibilitychange", syncBottomUserInfoOnVisible);
    }
    ...
    return () => {
      ...
      if (typeof window !== "undefined") {
        window.removeEventListener("focus", syncBottomUserInfo);
        window.removeEventListener("pageshow", syncBottomUserInfo);
      }

      if (typeof document !== "undefined") {
        document.removeEventListener("visibilitychange", syncBottomUserInfoOnVisible);
      }
...
/*
【今回このファイルで修正したこと】
1. タブ復帰時に LeftRail 最下部のアカウント表示を再同期するため、focus / pageshow / visibilitychange の再取得入口を追加しました。
2. mount時と auth event時だけに限定されていた session/profile 同期を、SPタブ復帰時にも走るようにしました。
3. 本文採用、スレッド切替、送信処理、メモリーズ取得、DB保存・復元、HOPY唯一の正、状態値 1..5 / 5段階には触っていません。
*/
