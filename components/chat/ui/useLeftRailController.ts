...
function useThreadsViewModel(params: {
  ...
  const activeThreadPropId = React.useMemo(() => {
    return readThreadId(activeThreadProp);
  }, [activeThreadProp]);

  const activeIdSafe = React.useMemo(() => {
    const id = String(activeThreadId ?? "").trim();
    return id || activeThreadPropId;
  }, [activeThreadId, activeThreadPropId]);
...
/*
【今回このファイルで修正したこと】
1. activeThreadId が空・未同期の場合でも、activeThreadProp.id を activeIdSafe の補助として使えるようにしました。
2. 本文側から渡っている activeThread と左カラムの Current Chat 表示条件が切れにくいようにしました。
3. activeThreadProp.id 読み取り専用の readThreadId() を追加しました。
4. スレッド選択、New Chat、状態の再判定、state_changed の再計算、HOPY回答○、Compass、DB保存・復元には触っていません。
*/
