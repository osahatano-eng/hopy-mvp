...
type Props = {
  ...
  activeThreadState: HopyState | null | undefined;
...
function ActiveThreadBadge({
  ...
  const normalizedActiveThreadState = React.useMemo(() => {
    return normalizeHopyState(activeThreadState ?? null);
  }, [activeThreadState]);
...
/*
【今回このファイルで修正したこと】
1. activeThreadState の受け取り型を HopyState | null | undefined に広げました。
2. normalizeHopyState() へ渡す前に undefined を null へそろえるようにしました。
3. 状態の再判定や 0..4 変換は追加せず、表示責務だけに留めました。
*/
