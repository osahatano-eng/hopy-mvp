...
export type ChatSendFutureChainDisplay = {
...
export type ChatSendRequestExecutionApiResponse<TState> =
  ConfirmedAssistantApiResponse<TState> & {
    ...
    future_chain_display?: ChatSendFutureChainDisplay | null;
  };
...
export async function runChatSendRequestExecution<TState>(args: {
  ...
  try {
    ...
    const futureChainDisplay = payload.future_chain_display ?? null;
...
【今回このファイルで修正したこと】
- API返却JSONの future_chain_display を型に追加した。
- runChatSendRequestExecution(...) の返却結果に futureChainDisplay を追加した。
- future_chain_display を再判定せず、payload から受け取った値をそのまま UI 側へ渡せる形にした。
- future_chain_persist の既存中継は維持した。
