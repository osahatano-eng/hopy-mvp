...
/*
【今回このファイルで修正したこと】
1. useThreadSwitch.ts に追加していた visibilitychange / focus / pageshow のタブ復帰再読込入口を削除しました。
2. lastResumeReloadAtRef と reloadActiveThreadMessages を削除しました。
3. タブ復帰時の親責務を useChatInit.ts 側へ戻しました。
4. useThreadSwitch.ts は hopy:select-thread と activeThreadId 変更時の messages 再読込だけを担当する形へ戻しました。
5. profile / plan 取得、threads取得、送信、MEMORIES には触れていません。
6. confirmed payload、state_changed、HOPY回答○、Compass、DB保存/復元、状態1..5 の唯一の正には触れていません。
*/
// /components/chat/lib/useThreadSwitch.ts