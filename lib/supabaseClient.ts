...
/*
【今回このファイルで修正したこと】
1. bindSupabaseAuthWake の wake 内で getSession() だけで終わらず、refresh_token がある場合に refreshSession() を実行するようにしました。
2. タブ復帰時に Supabase client 側の in-memory session / token が古いまま残る可能性に対して、明示的な session refresh を追加しました。
3. Supabase client の生成設定、storage一本化、PKCE、realtime、LeftRail表示、本文採用、スレッド切替、送信処理、メモリーズ取得、DB保存・復元、HOPY唯一の正、状態値 1..5 / 5段階には触っていません。
*/
