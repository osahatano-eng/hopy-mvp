...
async function runWithoutNavigatorLock<R>(
  name: string,
...
export const supabase: SupabaseClient =
  ...
  createClient(url, anon, {
    auth: {
      ...
      // ✅ PC Chrome / PWA / DevTools 復帰時の Navigator LockManager 詰まりを避ける
...
/*
【今回このファイルで修正したこと】
1. Supabase Auth の Navigator LockManager 経路を使わない runWithoutNavigatorLock() を追加しました。
2. auth.lock に runWithoutNavigatorLock を指定し、PC Chrome / PWA / DevTools 復帰時に "lock:hopy_auth" の取得待ちで止まる経路を避けました。
3. persistSession / autoRefreshToken / detectSessionInUrl / flowType / storage / storageKey は維持しました。
4. localStorage 正本、memory fallback の方針は維持しました。
5. HOPY唯一の正、confirmed payload、state_changed、HOPY回答○、Compass、状態値 1..5 / 5段階、DB保存・復元には触れていません。
*/
