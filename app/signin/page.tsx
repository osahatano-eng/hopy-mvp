...
function isSafeExchangeFallbackMessage(message: string): boolean {
  ...
  return (
    ...
    s.includes("auth code") ||
    s.includes("code already used") ||
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // - まずこのファイル自身で code exchange を明示実行する
        // - それでも未確定なら短時間だけ session 確定待ちを行う
        if (hasCode) {
...
/*
【今回このファイルで修正したこと】
/signin 復帰後に code が付いているのに session が確定しない経路を止めるため、
detectSessionInUrl 任せではなく、このファイルで exchangeCodeForSession(code) を明示実行するように修正しました。
exchange に失敗した場合でも即白固定にせず、短時間の session 確定待ちへ安全にフォールバックするようにしました。
*/
