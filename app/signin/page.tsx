...
export default function SignInPage() {
  ...
  const login = async () => {
    ...
    try {
      ...
      if (nextUrl) {
        pushDebug("login:redirect");
        location.assign(nextUrl);
        return;
      }

      pushDebug("login:no-url");
      setBusy(false);
    } catch (e) {
      const message = errText(e);

      if (isInvalidRefreshTokenErrorMessage(message)) {
        pushDebug("login:invalid-refresh");
        try {
          await resetLocalSessionArtifacts("login-invalid-refresh");
        } catch {}
      } else {
        pushDebug(message ? `login:err:${message}` : "login:err");
      }
...
/*
【今回このファイルで修正したこと】
1. signInWithOAuth() の戻り値から data.url を受け取り、Google への遷移を明示実行するようにしました。
2. ログイン開始失敗時に debugLine へ login:err を残すようにしました。
3. OAuth 復帰後の code 交換や /chat 遷移ロジックには触っていません。
*/
