...
/*
【今回このファイルで修正したこと】
- OpenAI completion の返り値が stream を含む union 型でも安全に扱えるよう、choices を持つ completion だけを通す型ガードを追加しました。
- readCompletionContent(...) の completion.choices 直接参照をやめ、安全な型ガード経由に修正しました。
- buildEmptyJsonContentErrorMessage(...) 内の completion.choices 直接参照も同じ型ガード経由に修正しました。
