...
function isLowValueConfirmedMemoryCandidate(
  ...
  const compactBody = compactMemoryText(body);
  ...
  const isAutoSupportContext =
    sourceType === "auto" && memoryType === "support_context";

  if (
    isAutoSupportContext &&
    includesAny(compactBody, lightInputWords) &&
    includesAny(compactBody, genericAutoFrames)
  ) {
    return true;
  }

  if (compactBody.length <= 16 && includesAny(compactBody, lightInputWords)) {
    return true;
  }
...
【今回このファイルで修正したこと】
- handleMemoryWrite(...) へ渡す前に、明らかに保存価値がない confirmedMemoryCandidates を除外する入口フィルタを追加した。
- 「おはよう」「こんにちは」「ありがとう」「OK」など軽い入口・相づち由来の auto support_context が、「整理や確認を求めている」などの薄い汎用文として保存されるケースを skip するようにした。
- filteredCandidates が空になった場合は handleMemoryWrite(...) を呼ばず、memoryWrite debug に low_value_confirmed_memory_candidates と mem_low_value_skipped_count を残すようにした。
