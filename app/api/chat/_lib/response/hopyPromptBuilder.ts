...
/*
【今回このファイルで修正したこと】
- 低シグナル入口入力に加えて、短文の軽い感想・軽い前向き発話・軽い応援でも state_changed を立てない指示を強化しました。
- state 5 を許可する条件を、明確な決定・行動開始・方針確定・強い意志表明がある場合だけにさらに明確化しました。
- buildIdentitySection / buildTransitionSection / buildThreeStepStructureSection / buildGenerationRulesSection / buildUserInputSection に、軽い短文と明確な決定表明を分離するルールを追加しました。
- 短文なら短文で返す挙動は残しつつ、短文を過大解釈して 5 / Compass に飛ばしにくくしました。
- 下流で補正せず、上流の確定意味ペイロード生成条件だけを修正する方針を維持しました。
*/
