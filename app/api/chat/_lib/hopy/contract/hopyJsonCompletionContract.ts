...
import type { OpenAIChatMessage } from "../../route/openaiExecution";
...
/*
【今回このファイルで修正したこと】
- /app/api/chat/_lib/route/openaiExecution.ts に残っている JSON契約検証責務を受け取る新規ファイルとして作成しました。
- completion content 読み取り、空 content 検出、JSON object parse、top-level key 検証、state 1..5 検証、state_changed 検証、Compass 契約検証をこのファイルへまとめました。
- state値は 1..5 / 5段階のまま維持し、0..4 前提にはしていません。
- HOPY唯一の正、Compass判定条件、DB保存復元、OpenAI実行処理、messages 組み立て処理はこのファイルでは再判定・再生成していません。
*/
