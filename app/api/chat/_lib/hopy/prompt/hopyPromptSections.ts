...
export function buildHopyGenerationRulesSection(
  ...
  if (explicitForwardCommitment) {
    return [
      ...
      "- 低シグナル入力以外では、できるだけ『HOPYはこう考えます』に相当する方向を残すこと。",
      ...planSpecificRules,
    ].join("\n");
  }
...
/*
【今回このファイルで修正したこと】
- hopyPromptBuilder.ts から分離するための、HOPY prompt section 専用ファイルを新規作成した。
- HOPY回答の中核である「理解 → 気づき → 方向 → なぜならば」を JSON契約と唯一の正の直後に置ける構成にした。
- developerPrompt の理想順を、JSON契約 → 唯一の正 → HOPY回答中核 → 文脈 → 今回入力 → 状態別policy → 状態遷移 → 密度/長さ → plan → 最終生成ルール に整理した。
- hopyPromptBuilder.ts 側が次回このファイルを読み込むだけに寄せられるよう、buildHopyDeveloperPromptFromSections を追加した。
