...
export function buildHopyPolicySection(
  ...
  if (lowSignal) {
    return [
      ...
      "- 低シグナル入口入力では、ユーザー自身の状態変化が会話上で明示されていない限り、current_phase / state_level は入力前参考状態の維持を基本にすること。",
      "- 低シグナル入口入力では、原則として state_changed=false にすること。",
      "- トーン調整は最小限にとどめ、本文を短く自然に返すこと。",
      "- 行動開始・方針確定・決定状態として読まないこと。",
      "- HOPY自身の返答で安心させたことを、ユーザーの決定・整理完了・状態変化として扱わないこと。",
      "",
      "低シグナル入力での回答目的:",
      "- 会話の入口として自然に受けること。",
      "- 過大解釈を避けること。",
...
export function buildHopyTransitionSection(args: {
  ...
  if (lowSignal) {
    return [
      ...
      "- 低シグナル入口入力では、原則として current_phase / state_level を prev_phase / prev_state_level と同じ値にすること。",
      "- 低シグナル入口入力では、原則として state_changed=false にすること。",
      "- 低シグナル入口入力だけを根拠に、current_phase / state_level を大きく上げないこと。",
      "- 低シグナル入口入力だけを根拠に、current_phase / state_level を 5 へ上げないこと。",
      "- 参考状態や参考上限目安が高くても、それを今回ターンの決定根拠にしないこと。",
      "- HOPY自身の返答で『落ち着いた』『受け止めた』と表現しても、それをユーザーの決定・整理完了・状態変化の根拠にしないこと。",
      "- Plus / Pro でも、低シグナル入口入力だけでは Compass を必要とする状態変化を作らないこと。",
...
export function buildHopyAnswerStructureSection(userInput: string): string {
  ...
  if (lowSignal) {
    return [
      ...
      "- HOPY自身が安心させる返答をしたことを、ユーザーの状態変化や決定完了として扱わないでください。",
    ].join("\n");
  }
...
export function buildHopyStateDensitySection(
  ...
  if (lowSignal) {
    return [
      ...
      "- 入力前参考状態が整理(3)・収束(4)・決定(5)でも、それは本文トーンの参考に留め、今回ターンの状態変化根拠にしないでください。",
      "- 決定・実行開始・整理完了として読まないでください。",
      "- HOPYの返答で落ち着かせたことを、ユーザーの決定や状態前進として扱わないでください。",
      "- 短く自然に返すことを優先してください。",
    ].join("\n");
  }
...
export function buildHopyGenerationRulesSection(
  ...
  if (lowSignal) {
    return [
      ...
      "- 今回のユーザー入力を最優先し、HOPY自身が生成した返答の安心感や落ち着き表現を根拠に、その回の current_phase / state_level / state_changed を進めないこと。",
      "- prev_phase / prev_state_level には入力前参考状態を入れ、低シグナル入口入力では current_phase / state_level も原則として prev_phase / prev_state_level と同じ値にすること。",
      "- 低シグナル入口入力では、原則として state_changed=false にすること。",
      "- 低シグナル入口入力で current_phase / state_level を変えてよいのは、ユーザー自身が明確な意味入力・決断・行動意思・方針確定を追加で示した場合だけです。",
      ...
      "- Plus / Pro でも、低シグナル入口入力だけでは Compass を返さないこと。",
      "- 挨拶や軽い短文では、短く自然に返すこと。",
      "- 深い気づきや強い方向提示を入れないこと。",
...
【今回このファイルで修正したこと】
- 低シグナル入口入力では、current_phase / state_level を原則 prev_phase / prev_state_level と同じ値にし、state_changed=false にするルールを明記した。
- 低シグナル入口入力だけを根拠に current_phase / state_level を 5 へ上げないルールを明記した。
- HOPY自身の返答で安心させたこと、落ち着かせたこと、受け止めたことを、ユーザーの状態変化・決定・整理完了として扱わないルールを追加した。
- Plus / Pro でも、低シグナル入口入力だけでは Compass を返さないルールを追加した。
