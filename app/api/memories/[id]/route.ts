...
export async function PATCH(req: Request, ctx: RouteCtx) {
  ...
  const nextTextRaw = body?.text ?? body?.content;
  const hasText = typeof nextTextRaw === "string";
  const nextText = hasText ? String(nextTextRaw).trim() : "";

  const hasDeletedAt = Object.prototype.hasOwnProperty.call(body ?? {}, "deleted_at");
  const deletedAt =
    hasDeletedAt ? (body.deleted_at === null ? null : String(body.deleted_at)) : undefined;

  if (!hasText && !hasDeletedAt) {
  ...
  const patch: any = {};
  if (hasText) patch.content = nextText;
  if (hasDeletedAt) patch.deleted_at = deletedAt;
...
export async function DELETE(req: Request, ctx: RouteCtx) {
  ...
  if (hard) {
    return json(400, { ok: false, error: "hard_delete_disabled" });
  }
